...
const Dlogin=()=>{
  const navigate = useNavigate();
    ...
    useEffect(()=>{
      async function getallcon(){
          try{
              const conductor=await axios.get("http://127.0.0.1:8000/routes/driver/?format=json")
    ...
    const handleLogin = () => {
      ...
      const c=con.find((e)=>e.D_id===num);
      console.log(typeof c.salary);
      const cp=con.find((e)=>e.D_name===password);
      if (c && cp && c.D_id === cp.D_id) {
        // Successful login, navigate to another page
        setresult(c);
        navigate(`/sucesslogin/${c.D_id}/${c.salary}/${c.working_shift}`);
      } else {
  ...
  Your shift is  {result.working_shift} ;
</div>
</div>
        ):(
